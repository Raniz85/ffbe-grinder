 ...
 * @since 2017-05-09.
 */
class DecrementCounterAction(val name: String, val amount: Int = 1, repeat: Boolean = false): AbstractGameAction(repeat) {
    override fun execute(controller: DeviceController, counters: MutableMap<String, Int>) {
        counters.compute(name) { k, v -> (v ?: 0) - amount }
    }

    override fun toString() = "Subtract ${amount} from counter ${name}"
}
